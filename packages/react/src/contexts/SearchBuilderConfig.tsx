...
export function useSearchBuilderConfig<T extends TranslationStrings, I extends IconSet>() {
  return useContext(SearchBuilderConfigContext) as SearchBuilderConfigContextState<T, I> | null;
}

export function SearchBuilderConfigProvider<T extends TranslationStrings, I extends IconSet>({
  translations,
  icons,
  children,
}: SearchBuilderConfigProviderProps<T, I>): React.ReactElement {
  const value = useMemo<SearchBuilderConfigContextState<T, I>>(() => {
    function icon(key: keyof I): ReactNode {
      return icons[key];
    }

    const translate: GenericTranslator<T> = (key, options) => {
      return translateFrom(translations, key, options);
    };
